...
-- stats & locations
local endpoint_underground = EntityGetClosestWithTag( x, y, "ending_sampo_spot_underground" )
local endpoint_mountain = EntityGetClosestWithTag( x, y, "ending_sampo_spot_mountain" )

GamePrint(endpoint_mountain .. " below: " .. endpoint_underground)
...
if( orb_count < 33 
	...
	if( #endpoint_mountain > 0 ) then
		local ex, ey = EntityGetTransform( endpoint_mountain )
...
if( doing_newgame_plus == false ) then
	...
	if( orb_count >= 33 ) then
		...
		if ( #endpoint_underground > 0 ) then
			local ex, ey = EntityGetTransform( endpoint_underground )
	...
	elseif ( #endpoint_underground > 0 ) then
		-- NORMAL ENDING
		local endpoint_id = endpoint_underground
		local ex, ey = EntityGetTransform( endpoint_id )
	...
	elseif ( #endpoint_mountain > 0 ) then
		-- SECRET ENDING
		local endpoint_id = endpoint_mountain
		local ex, ey = EntityGetTransform( endpoint_id )
